...
def get_multiqc_input(wildcards):
    ...
    data = list(itertools.chain.from_iterable([multiext(x, "_tcperutr.csv", "tcperreadpos.csv") for x in expand("results/{sample}_slamdunk_mapped_filtered", sample=samples["sample_name"])]))
    return data

def get_fastqc(wildcards):
    return expand("results/qc/fastqc/{sample}_fastqc.zip", sample=samples["sample_name"])
